...
func helper(root *TreeNode, depth, now int) [][]string {
	ret := [][]string{}
	if root == nil || (root.Left == nil && root.Right == nil) {
		for i := 0; i < depth-now; i++ {
			ret = append(ret, make([]string, (1<<uint32(depth-now))-1))
		}
		if root != nil {
			ret[0][len(ret[0])/2] = strconv.Itoa(root.Val)
		}
		printTreeNode(root)
		for i := 0; i < len(ret); i++ {
			fmt.Print(len(ret[i]))
		}
		fmt.Println(len(ret[0]), ret)
		return ret
	}
	left, right := helper(root.Left, depth, now+1), helper(root.Right, depth, now+1)
	ret = append(ret, make([]string, len(left[0])+len(right[0])+1))
	ret[0][len(left[0])] = strconv.Itoa(root.Val)
	for i := 0; i < len(left); i++ {
		left[i] = append(left[i], "")
...
func main() {
	fmt.Println(printTree(buildTree([]int{1, 2, 5, 3, -1, -1, -1, 4})))
}
