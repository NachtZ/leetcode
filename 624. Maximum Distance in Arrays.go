...
func maxDistance(arrays [][]int) int {
	...
	for i := 1; i < len(arrays); i++ {
		ret = max(abs(arrays[i][len(arrays[i])-1]-minv), ret)
		ret = max(abs(maxv-arrays[i][0]), ret)
		minv = min(minv, arrays[i][0])
		maxv = max(maxv, arrays[i][len(arrays[i])-1])
	}
	return ret
}
func main() {
	fmt.Println(maxDistance([][]int{[]int{-3, -3}, []int{-3, -2}}))
}
