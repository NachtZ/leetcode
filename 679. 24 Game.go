...
func helper(nums []float64) bool {
	...
	for i := 0; i < len(nums); i++ {
		for j := 0; j < i; j++ {
			if i == 1 && j == 0 && len(nums) == 2 && math.Abs(nums[0]-24) <= 0.001 {
				fmt.Println("Here")
			}
			m := []float64{(nums[i] + nums[j]), (nums[i] - nums[j]), (nums[j] - nums[i]), (nums[i] * nums[j])}
			if math.Abs(nums[j]) > 0.00001 {
				m = append(m, nums[i]/nums[j])
				//m[(nums[i] / nums[j])] = true
			}
			if math.Abs(nums[i]) > 0.00001 {
				m = append(m, nums[j]/nums[i])
				//m[(nums[j] / nums[i])] = true
			}

			for _, v := range m {
				tmp := make([]float64, len(nums))
				copy(tmp, nums)
				tnum := append(tmp[0:j], tmp[j+1:i]...)
				tnum = append(tnum, tmp[i+1:]...)
				tnum = append(tnum, v)
				ret := helper(tnum)
				if ret == true {
					fmt.Println(tnum)
...
func judgePoint24(nums []int) bool {
	tnum := []float64{float64(nums[0]), float64(nums[1]), float64(nums[2]), float64(nums[3])}
	return helper(tnum)
}
func main() {
	fmt.Println(judgePoint24([]int{1, 5, 9, 1}))
