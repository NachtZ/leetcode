...
func Constructor(c string) StringIterator {
	num, nums := 0, []int{}
	ctx := ""
	for i := 0; i < len(c); i++ {
		if c[i] <= '9' && c[i] >= '0' {
...
func (this *StringIterator) Next() byte {
	if this.idx >= len(this.num) {
		return ' '
	}
	this.cnt++
	ret := this.ctx[this.idx]
	if this.cnt == this.num[this.idx] {
		this.cnt = 0
		this.idx++
	}
	return ret
...
func (this *StringIterator) HasNext() bool {
	return len(this.num) > this.idx
}

func main() {
	t := Constructor("L1e2t1C1o1d1e1")
	fmt.Println(string(t.Next()))
	fmt.Println(string(t.Next()))
	fmt.Println(string(t.Next()))
	fmt.Println(string(t.Next()))
	fmt.Println(string(t.Next()))
