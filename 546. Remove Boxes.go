...
/**************************************************************/
func helper(dp [][][]int, boxes []int, i, j, k int) int {
	if i > j {
		return 0
	}
	if i == j {
	...
	for m := i + 1; m <= j; m++ {
		if boxes[i] == boxes[m] {
			tmp = max(tmp, helper(dp, boxes, i+1, m-1, 1)+helper(dp, boxes, m, j, k+1))
		}
	}
	dp[i][j][k] = tmp
	return tmp
...
func removeBoxes(boxes []int) int {
	...
	for i := 0; i < len(boxes); i++ {
		...
		for j := 0; j < len(boxes); j++ {
			dp[i][j] = make([]int, len(boxes))
		}
	}
	return helper(dp, boxes, 0, len(boxes)-1, 1)
}
