...
func isValidTag(code1, code2 string) bool {
	if len(code1)+1 != len(code2) {
		return false
	}
	for i, v := range code1 {
		if !(v <= 'Z' && v >= 'A' || v <= '9' && v >= '1') {
...
func isValid(code string) bool {
	re, _ := regexp.Compile("<!\\[CDATA\\[.*?\\]\\]>")
	str := re.ReplaceAllString(code, "t") //clean any cdata
	//	fmt.Println(str)
	//golang v1.6 regexp do not support ahead. So can not use regexp later.
	//re, err := regexp.Compile("<([A-Z]{1,9})>[^<]*</\\1>")
	re, err := regexp.Compile("<([A-Z]{1,9})>[^<]*<(/[A-Z]{1,9})>")
	if err != nil {
		fmt.Println(err)
		return false
	}
	...
	for pre != str {
		...
		//	fmt.Println(str)
		if len(strs) == 3 && isValidTag(strs[1], strs[2]) == false {
			return false
		}
...
func main() {
	fmt.Println(isValid("<![CDATA[ABC]]><TAG>sometext</TAG>"))
