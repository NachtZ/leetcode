...
func helper(root *TreeNode, depth int, idx int) {
	if root == nil {
		return
	}
	if len(width) <= depth {
		width = append(width, [2]int{idx, idx})
	} else {
		if width[depth][0] > idx {
			width[depth][0] = idx
		} else if width[depth][1] < idx {
			width[depth][1] = idx
		}
		if width[depth][1]-width[depth][0]+1 > maxw {
			maxw = width[depth][1] - width[depth][0] + 1
		}
	}
	helper(root.Left, depth+1, 2*idx)
	helper(root.Right, depth+1, 2*idx+1)
}
func widthOfBinaryTree(root *TreeNode) int {
	width = [][2]int{[2]int{1, 1}}
	maxw = 1
...
func main() {
	fmt.Println(widthOfBinaryTree(buildTree([]int{1, 3, -1, 5, 3})))
}
