...
func shoppingOffers(price []int, special [][]int, needs []int) int {
	...
	for t := 0; t < len(special); t++ {
		prices := special[t][len(special[t])-1]
		special[t] = special[t][:len(special[t])-1]
		for len(special[t]) < 6 {
			special[t] = append(special[t], 0)
		}

		for i := special[t][0]; i <= needs[0]; i++ {
			for j := special[t][1]; j <= needs[1]; j++ {
				for k := special[t][2]; k <= needs[2]; k++ {
					for l := special[t][3]; l <= needs[3]; l++ {
						for m := special[t][4]; m <= needs[4]; m++ {
							for n := special[t][5]; n <= needs[5]; n++ {
								dp[i][j][k][l][m][n] = min(dp[i][j][k][l][m][n], dp[i-special[t][0]][j-special[t][1]][k-special[t][2]][l-special[t][3]][m-special[t][4]][n-special[t][5]]+prices)
							}
						}
					}
				}
...
func main() {
	fmt.Println(shoppingOffers([]int{2, 5}, [][]int{[]int{3, 0, 5}, []int{1, 2, 10}}, []int{3, 2}))
}
