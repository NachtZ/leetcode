...
func dfs(root *TreeNode, minValue int) int {
	if root == nil {
		return -1
	}
	...
	left := dfs(root.Left, minValue)
	right := dfs(root.Right, minValue)
	if left == -1 {
		return right
	}
	if right == -1 {
...
func main() {
	fmt.Println(findSecondMinimumValue(buildTree([]int{2, 2, 5, -1, -1, 5, 7})))
}
