...
func check(nums []int) bool {
	...
	size := nums[len(nums)-1] - nums[0] + 1
	dp, one, two, all := make([]int, size), make([]int, size+1), make([]int, size+1), make([]int, size+1)
	for i := 0; i < len(nums); i++ {
		dp[nums[i]-nums[0]]++
	}
	for i := 0; i < size; i++ {
...
func main() {
	fmt.Println(isPossible([]int{1, 2, 3, 4, 5}))
}
